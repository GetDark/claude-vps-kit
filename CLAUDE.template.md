...
**ГЛАВНОЕ ПРАВИЛО: запрещено начинать вёрстку, пока не изучены аналоги, не предложено ~10 вариантов концепций и пользователь не выбрал одну.**
...
3. mcp__duckduckgo__search("НИША website design 2025 modern")  → 5–10 реальных аналогов
4. Зафиксировать: лучшее из каждого аналога + штампы которые у ВСЕХ (= избегать)
5. Предложить ~10 концепций (7–15) — каждая с палитрой, layout, hero, уникальной фишкой
6. (пользователь выбирает)
7. /gdd:design         → синтез: выбранная концепция + лучшие детали из остальных
8. /gdd:audit          → проверка: итог не должен быть похож НИ НА ОДИН аналог
9. /visual-qa          → браузер, desktop + mobile
10. mcp__memory__create_entities(...)  → сохранить дизайн-решения
```

### КРИТИЧЕСКОЕ ПРАВИЛО: итоговый дизайн не должен быть похож ни на один аналог

Аналоги — источник ИДЕЙ, а не шаблонов. Перед вёрсткой каждого блока задавать вопрос:
> «Если открыть любой сайт из этой ниши — там такой же hero? Такая же структура? Такое же меню?»
> Если ответ "да" — переделать.

### HERO-БЛОК: главная зона риска (чаще всего получается одинаковым)

**Запрещено по умолчанию** (только по явному запросу пользователя):
- Заголовок слева + фото справа (split 50/50) — самый распространённый шаблон
- Баннер с тёмным полупрозрачным оверлеем + белый текст по центру
- Карусель/слайдер как hero
- Стоковое фото офиса / рукопожатия / ноутбука как главный визуал
- Крупный заголовок по центру + подзаголовок + две кнопки CTA (дефолт Bootstrap/Tailwind)

**Нестандартные hero-структуры (выбирать из этого списка):**
| Вариант | Описание |
|---------|----------|
| Типографический fullscreen | Только текст, огромный шрифт, цвет фона делает всё |
| Видео / canvas-анимация | Видео или WebGL на весь экран, 1–2 строки поверх |
| Интерактивный курсор | Элементы реагируют на движение мыши / hover |
| Коллаж-сетка | Мозаика из фото/карточек, заголовок вписан между ними |
| Диагональный split | clip-path делит экран по диагонали на 2 зоны |
| Бегущая строка (marquee) | Лента с ключевыми словами как визуальный акцент |
| SVG-иллюстрация / анимация | Кастомная иллюстрация без фото |
| 3D / parallax слои | CSS 3D transform, глубина при скролле |
| Outline-текст + нормальный | Крупный outline (обводка) + читаемый текст поверх |
| Анимированные цифры/статистика | Счётчики как главный визуал hero |
...
| Hero layout | Типографический / Видео-фон / Коллаж / Диагональный / Интерактивный |
...
### Стоковые фото (брать самостоятельно, не спрашивать пользователя):

Если для дизайна нужны изображения — брать подходящие по смыслу со стоков без запроса к пользователю. Спрашивать только если нужны **уникальные/брендовые** материалы (логотип, фото команды, конкретный продукт).

**Unsplash** (бесплатно, без ключа):
```
https://images.unsplash.com/photo-{PHOTO_ID}?w=1400&q=80
```
Поиск через fetch:
```
...
**Pexels** (альтернатива, тоже бесплатно):
```
https://www.pexels.com/search/KEYWORD/
```

**Picsum** (заглушки с реальными фото, когда нужен просто размер):
```
https://picsum.photos/1200/800
https://picsum.photos/seed/WORD/1200/800   ← стабильный ID по слову
```

**Правило выбора фото:** подбирать по смыслу ниши — для барбершопа брать фото стрижки, для ресторана — еду, для IT — не офис с рукопожатием, а код/интерфейсы/абстракцию. Избегать Generic-стока (улыбающиеся люди в костюмах, рукопожатия, ноутбуки на белом фоне).
